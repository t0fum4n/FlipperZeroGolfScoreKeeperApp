...
// all credits to xMasterX for the core logic
...
static void draw_labeled_button(Canvas *canvas, uint8_t origin_x, uint8_t origin_y, bool selected, const char *label)
{
    const uint8_t box_width = 18;
    const uint8_t box_height = 12;
    const uint8_t box_x = origin_x;
    const uint8_t box_y = origin_y - 8;

    canvas_set_color(canvas, selected ? ColorBlack : ColorWhite);
    canvas_draw_box(canvas, box_x, box_y, box_width, box_height);
    canvas_set_color(canvas, selected ? ColorWhite : ColorBlack);
    canvas_draw_frame(canvas, box_x, box_y, box_width, box_height);

    uint8_t label_offset = 3;
    ...
    canvas_draw_str(canvas, box_x + label_offset, origin_y, label);
}
...
static void uart_text_input_view_draw_callback(Canvas *canvas, void *_model)
{
    ...
    if (model->valadator_message_visible)
    {
        ...
        canvas_draw_str(canvas, 14, 28, "!");
        elements_multiline_text(canvas, 28, 20, furi_string_get_cstr(model->validator_text));
        canvas_set_font(canvas, FontKeyboard);
    }
}
